...
// void check_swap_in(struct usm_event *event) {
//     if (usedMemory < poolSize / 2.5) {
//         pthread_t worker;
//         int nr = (swappedMemory / 2) / SYS_PAGE_SIZE;
//         if (nr > 0) {
//             if (pthread_create(&worker, NULL, swap_in_list_and_commit, &nr) != 0) {
...
//         pthread_mutex_lock(&procDescList[event->origin].swpLock);
//         if (list_empty(&procDescList[event->origin].swapList)) {
//             printf("Proc. used list empty!\n");
//             pthread_mutex_unlock(&procDescList[event->origin].swpLock);
//             return;
//         }
//         pthread_mutex_unlock(&procDescList[event->origin].swpLock);

//         struct to_swap *toSwapNode = malloc(sizeof(struct to_swap));
//         if (toSwapNode == NULL) {
//             perror("malloc");
//             // Handle malloc error
//             return;
...
//         if (toSwapNode->swapDevice->swap_dev_ops.swap_in(toSwapNode)) {
// #ifdef DEBUG
//             printf("Failed to swap in used page.\n");
// #endif
//         } else {getchar();
//     getchar(); // Potential debugging pause
// }
